output.data = {
  // === Test data for registration
  NEW_USER_FIRST_NAME: "John",
  NEW_USER_LAST_NAME: "Doe",
  NEW_USER_EMAIL: "john.doe@example.com",
  NEW_USER_PHONE: "1235551234",
  NEW_USER_PASSWORD: "password345",
  // === Test data for sign in and use
  RECIPIENT_NAME: "Robert",
  RECIPIENT_EMAIL: "patient@careconnect.demo",
  RECIPIENT_PASSWORD: "password123",
  CAREGIVER_NAME: "Sarah",
  CAREGIVER_EMAIL: "caregiver@careconnect.demo",
  CAREGIVER_PASSWORD: "password123",
  // === Health log test data
  TEST_MOOD_EMOJI: "🤩",
  TEST_MOOD_VALUE: "Ecstatic",
  TEST_BP_SYSTOLIC: "119",
  TEST_BP_DIASTOLIC: "79",
  TEST_HEART_RATE: "72",
  // === Note test data
  TEST_NOTE_TITLE: "Medication Side Effects",
  TEST_NOTE_CONTENT: "Experienced mild dizziness after morning dose",
};